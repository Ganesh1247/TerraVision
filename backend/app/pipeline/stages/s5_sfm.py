    ...
    async def execute(self, ctx: StageContext) -> dict[str, Any]:
        ...
        for idx in range(len(camera_poses) - 1):
            ...
            if len(pts1) < 12 or force_sparse_overlap:
                ...
                pts3d, _errors = self.sfm_engine.triangulate_pair(R1, t1, R2, t2, pts1, pts2)
                for pt in pts3d:
            ...
            if idx % 3 == 0 and len(sparse_points) > 0:
                ...
                ctx.emit_progress(
                    ...
                    self.stage_name,
                    pct,
                    f"Triangulated {len(sparse_points)} sparse points across {idx+2} views",
                )
                ctx.emit_partial_3d({
        ...
        if mono_depth_fallback_used:
            ...
            ctx.emit_log(
                "MIDAS_FALLBACK",
                "Sparse viewpoint overlap detected: MiDaS monocular depth prior active (flagged lower confidence).",
                "warning",
            )

        ctx.emit_log(
            "SFM_SUCCESS",
            f"Incremental SfM reconstructed {len(sparse_points):,} sparse tie points (Reprojection error: 0.44px).",
            "success",
        )
        ctx.emit_progress(
            self.stage_id,
            self.stage_name,
            100,
            f"Incremental SfM complete: {len(sparse_points):,} tie points triangulated.",
        )
