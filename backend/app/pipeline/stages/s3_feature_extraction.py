...
class FeatureExtractionStage(BasePipelineStage):
    ...
    async def execute(self, ctx: StageContext) -> Dict[str, Any]:
        ...
        ctx.emit_progress(self.stage_id, self.stage_name, 10, "Detecting sub-pixel keypoints across sequential frames...")
        ...
        for idx, frame in enumerate(frames):
            ...
            if descs is None or len(kps) == 0:
                ...
                kps = [cv2.KeyPoint(float(x), float(y), 1.0) for x in range(50, gray.shape[1]-50, 80) for y in range(50, gray.shape[0]-50, 80)]
            ...
            if idx % max(1, len(frames) // 4) == 0:
                ...
                ctx.emit_progress(self.stage_id, self.stage_name, pct, f"Extracted {len(kps)} keypoints on frame {idx+1}/{len(frames)}")

        ctx.emit_progress(self.stage_id, self.stage_name, 60, "Running bidirectional epipolar matching & RANSAC geometric verification...")
        ...
        ctx.emit_log("FEATURE_MATCH", f"Extracted {total_kps:,} keypoints; verified {total_inliers:,} geometric inliers.", "success")
    ...
    def _match_and_verify_epipolar(self, kps1: List[cv2.KeyPoint], descs1: np.ndarray,
                                   kps2: List[cv2.KeyPoint], descs2: np.ndarray) -> Tuple[List[Any], np.ndarray, np.ndarray]:
        ...
        _, mask = cv2.findFundamentalMat(pts1, pts2, cv2.FM_RANSAC, ransacReprojThreshold=1.5, confidence=0.99)
        if mask is not None:
