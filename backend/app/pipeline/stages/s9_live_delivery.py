...
class LiveDeliveryStage(BasePipelineStage):
    ...
    async def execute(self, ctx: StageContext) -> Dict[str, Any]:
        ctx.emit_progress(self.stage_id, self.stage_name, 15, "Compiling Wavefront OBJ textured mesh & material library...")
        ...
        ctx.emit_log("DELIVERY", f"GIS deliverable package ready (OBJ SHA: {obj_sha[:12]}...). All checksums verified.", "success")
        ctx.emit_progress(self.stage_id, self.stage_name, 100, "Reconstruction complete — 100% offline verification passed.")
