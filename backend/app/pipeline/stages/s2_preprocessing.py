...
class PreprocessingStage(BasePipelineStage):
    ...
    async def execute(self, ctx: StageContext) -> Dict[str, Any]:
        ...
        ctx.emit_progress(self.stage_id, self.stage_name, 15, "Quality router inspecting frame photometric entropy & blur metrics...")
        ...
        if applied_enhancements["low_light"] > 0 or force_low_light:
            ...
            ctx.emit_log("ZERO_DCE", f"Zero-DCE/CLAHE contrast boost applied to {applied_enhancements['low_light']} frames.", "info")
        if applied_enhancements["dehazed"] > 0 or force_fog_haze:
            active_fallbacks.append("dehazing")
            ctx.emit_log("DCP_DEHAZE", f"Dark Channel Prior dehazing applied to {applied_enhancements['dehazed']} frames.", "info")
        if applied_enhancements["deblurred"] > 0 or force_motion_blur:
            active_fallbacks.append("deblurring")
            ctx.emit_log("WIENER_DEBLUR", f"Wiener vibration deconvolution applied to {applied_enhancements['deblurred']} frames.", "info")

        ctx.shared_state["active_fallbacks"] = list(set(ctx.shared_state.get("active_fallbacks", []) + active_fallbacks))
    ...
    def _quality_router(self, frame_rgb: np.ndarray, force_low_light: bool = False,
                        force_fog_haze: bool = False, force_motion_blur: bool = False) -> Tuple[np.ndarray, List[str]]:
        """
