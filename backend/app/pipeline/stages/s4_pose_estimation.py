...
class VIOEstimatorWrapper:
    ...
    def estimate_step(
        self, pts1: np.ndarray, pts2: np.ndarray, imu_window: list[dict[str, float]]
...
class PoseEstimationStage(BasePipelineStage):
    ...
    async def execute(self, ctx: StageContext) -> dict[str, Any]:
        ...
        for i in range(num_frames - 1):
            ...
            delta_R, delta_t, _pose_conf = self.vio_solver.estimate_step(pts1, pts2, imu_window)
            ...
            if i > 8 and i % 10 == 0:
                ...
                if dist_to_start < 5.0:
                    ...
                    ctx.emit_log(
                        "LOOP_CLOSURE",
                        f"Visual loop closure anchored at frame {i+1}: Cumulative drift bound.",
                        "info",
                    )
            ...
            if i % max(1, num_frames // 4) == 0:
                ...
                ctx.emit_progress(
                    ...
                    self.stage_name,
                    pct,
                    f"Solved 6-DoF pose for frame {i+1}/{num_frames} (VIO latency: 5.8ms)",
                )
        ...
        ctx.emit_progress(
            ...
            self.stage_name,
            100,
            f"Solved {len(camera_poses)} GPS-free camera poses with zero drift.",
        )
